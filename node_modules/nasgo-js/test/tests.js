...
var nasgo = require("../index.js");

describe("nasgo JS", function () {
	...
	it("should have properties", function () {
		...
		properties.forEach(function (property) {
			(nasgo).should.have.property(property);
		});
	});

	describe("crypto.js", function () {
		var crypto = nasgo.crypto;
	...
	describe("dapp.js", function () {
		...
		describe("#createDApp", function () {
			...
			var options = {
				"name": "nasgo-dapp-cctime",
				"link": "https://github.com/nasgoPlatform/nasgo-dapp-cctime/archive/master.zip",
				"category": 1,
				"description": "Decentralized news channel",
				"tags": "nasgo,dapp,demo,cctime",
			...
			describe("returned dapp", function () {
				var keys = nasgo.crypto.getKeys("secret");
				var secondKeys = nasgo.crypto.getKeys("secret 2");
				...
				it("should be signed correctly", function () {
					var result = nasgo.crypto.verify(trs);
					(result).should.be.ok;
				});
				...
				it("should be second signed correctly", function () {
					trs.amount = 0;
					var result = nasgo.crypto.verifySecondSignature(trs, secondKeys.publicKey);
					(result).should.be.ok;
				});
				...
				it("should be ok to verify bytes", function () {
					...
					var secret = 'secret1'
					var keys = nasgo.crypto.getKeys(secret)
					var signature = nasgo.crypto.signBytes(data1, keys)
					var result = nasgo.crypto.verifyBytes(data1, signature, keys.publicKey)
					result.should.be.ok

					var data2 = new Buffer('a1b2c3d4', 'hex')
					signature = nasgo.crypto.signBytes(data2, keys)
					result = nasgo.crypto.verifyBytes(data2, signature, keys.publicKey)
					result.should.be.ok
				})
			});
		});
	...
	describe("delegate.js", function () {
		...
		describe("#createDelegate", function () {
			...
			describe("returned delegate", function () {
				...
				it("should be signed correctly", function () {
					var result = nasgo.crypto.verify(trs, keys.publicKey);
					(result).should.be.ok;
				});
				...
				it("should not be second signed correctly now", function () {
					trs.amount = 100;
					var result = nasgo.crypto.verify(trs, secondKeys.publicKey);
					(result).should.be.not.ok;
				});
	...
	describe("transaction.js", function () {
		var transaction = nasgo.transaction;
		...
		describe("#createTransaction with second secret", function () {
			...
			var keys = nasgo.crypto.getKeys(secondSecret);
			...
			describe("returned transaction", function () {
				...
				it("should be second signed correctly", function () {
					var result = nasgo.crypto.verifySecondSignature(trs, keys.publicKey);
					(result).should.be.ok;
				});
	...
	describe("vote.js", function () {
		...
		describe("#createVote", function () {
			var createVote = vote.createVote,
				vt = null,
				publicKey = nasgo.crypto.getKeys("secret").publicKey,
			...
			describe("returned vote", function () {
				...
				it("should be signed correctly", function () {
					var result = nasgo.crypto.verify(vt);
					(result).should.be.ok;
				});

				it("should be second signed correctly", function () {
					var result = nasgo.crypto.verifySecondSignature(vt, nasgo.crypto.getKeys("second secret").publicKey);
					(result).should.be.ok;
				});
	...
	describe('crypto sha256 and address', function () {
		it('should be equal to the expected address', function () {
			nasgo.crypto.getAddress('7a91b9bfc0ea185bf3ade9d264da273f7fe19bf71008210b1d7239c82dd3ad20').should.be.equal('AFbYJhiJb3DXzHy5ZP24mKw21M2dCBJCXP')
			var publicKeyBuffer = new Buffer('7a91b9bfc0ea185bf3ade9d264da273f7fe19bf71008210b1d7239c82dd3ad20', 'hex')
			nasgo.crypto.getAddress(publicKeyBuffer).should.be.equal('AFbYJhiJb3DXzHy5ZP24mKw21M2dCBJCXP')
		})
	})
